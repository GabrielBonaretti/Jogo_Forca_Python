...
def remover_lista(lista_palavras_param, lista_dicas_param):
    ...
    '''
    if len(lista_palavras_param) <= 1:
        print("Erro! É necessário ter pelo menos uma palavra para jogar.")
        sleep(2)
        os.system('cls')
    else:
        lista_palavras_param.append("Sair")
        sys.path.append(os.path.realpath("."))
        menu_remover = [
            inquirer.List(
            "escolha",
            message="Palavras e opção 'Sair'",
            choices= lista_palavras_param,
            ),
        ]

        answers = inquirer.prompt(menu_remover)
        resposta = answers.get("escolha")
        lista_palavras_param.remove("Sair")

        for i in range(len(lista_palavras_param)):
            if resposta == lista_palavras_param[i - 1]:
...
def apagar_csv(lista_palavras_param, lista_dicas_param):
    ...
    :return:
    '''
    remover_lista(lista_dicas_param=lista_dicas_param, lista_palavras_param=lista_palavras_param)
